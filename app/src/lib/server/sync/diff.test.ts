...
describe('diffTables', () => {
	...
	it('does not report a row as changed when only object key order differs', () => {
		const aa = { ...empty, chipCosts: [{ rarity: 'Common', toGold: 5, toDiamond: 25, toRainbow: 40, toBeskar: 80 }] };
		const bb = { ...empty, chipCosts: [{ toBeskar: 80, toRainbow: 40, toDiamond: 25, toGold: 5, rarity: 'Common' }] };
